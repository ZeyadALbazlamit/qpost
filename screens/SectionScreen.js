...
export class SectionScreen extends React.Component {
  constructor(props) {
    super(props);
  ...
  async loadingInitialData() {
    const data = await Api.getPostDataFromCategory(this.state.id)
  ...
  async loadMoreArticle() {
    ...
    this.setState({pages: numberOfPage},
      async() => {
      const {id} = this.state
      const data = await Api.getMorePostDataFromCategory(id, this.state.pages)
  ...
  render() {
    ...
    const {loadmore, loading, postData} = this.state
    return (
      <Container isWhiteTheme={themState}>
        {
          ...
          >
            <View style = {styles.topBar}>
              <TouchableOpacity onPress = {() => this.props.navigation.pop()} style = {{paddingRight: 15}}>
                <FontAwesome name={'angle-right'} size={40} color={themState ? 'black' : 'white'} />
              </TouchableOpacity>
              <Text style = {themState ? CustomStyle.headerTitle : {...CustomStyle.headerTitle, color: 'white'}}>{title}</Text>
            </View>
            <TouchableOpacity 
            ...
            {this._renderSwipeBookingList(postData.slice(1, postData.length))}
